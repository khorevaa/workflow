...
Процедура ПриОшибкеВыполненияЗадачи()
	...
	Делегат = Делегаты.Создать(ОбработчикПриОшибке.Обработчик, ОбработчикПриОшибке.ИмяМетода);
...
Функция ПриОшибке(Обработчик, Знач ИмяМетодаОбработчика) Экспорт
	...
	МетодЗадачиЕсть = РефлекторОбработчика.ЕстьПроцедура(ИмяМетодаОбработчика, 1);
	...
	ДобавитьДополнительныйОбработчик("ПриОшибке", СтруктураОбработчика);
